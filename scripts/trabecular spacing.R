...
setwd("C:\\Users\\konst\\insybio\\OsteoNet\\trabecular spacing")
...
# Load the datasets
inputs <- read.csv("spacing_inputs.csv", header = TRUE, row.names = 1)
outputs <- read.csv("spacing_outputs.csv", header = TRUE, row.names = 1)
...
write.csv(scaling_params, "spacing_scaling_parameters.csv", row.names = FALSE)

scaled_data <- data.frame(ID = row.names(inputs), inputs_matrix)

# Add the outcomes and binary columns
scaled_data$Trabecular.spacing <- outputs$Trabecular.spacing
...
write.csv(scaled_data, "spacing_scaled.csv", row.names = FALSE)
...
# Train the model for Trabecular spacing
model <- glmnet(inputs_matrix, outputs_matrix, family = "gaussian", alpha = alpha_val)

# Save the model
saveRDS(model, "model_trabecular_spacing.rds")
cat("Model for Trabecular spacing saved as model_trabecular_spacing.rds\n")
...
write.csv(data.frame(Model = "trabecular_spacing", LambdaMin = lambda.min_inputs), "spacing_lambda.csv", row.names = FALSE)

coef(model, s = cvmodel$lambda.min)

# Save the cross-validated model
saveRDS(cvmodel, "cvmodel_trabecular_spacing.rds")
cat("Cross-validated model saved as cvmodel_trabecular_spacing.rds\n")
...
miRData <- read.csv("spacing_scaled.csv", header = TRUE)
...
write.csv(df, file = "spacing_predictions.csv", row.names = FALSE)
...
for (feature in features) {
  ...
  cor_test_actual <- cor.test(inputs[[feature]], scaled_data$Trabecular.spacing, method = "spearman")
  ...
  actual_plot <- ggplot(data.frame(x = inputs[[feature]], y = scaled_data$Trabecular.spacing), aes(x = x, y = y)) +
