...
setwd("C:\\Users\\konst\\insybio\\OsteoNet\\connectivity density")
...
# Load the datasets
inputs <- read.csv("connectivity_inputs.csv", header = TRUE, row.names = 1)
outputs <- read.csv("connectivity_outputs.csv", header = TRUE, row.names = 1)
...
write.csv(scaling_params, "density_scaling_parameters.csv", row.names = FALSE)

scaled_data <- data.frame(ID = row.names(inputs), inputs_matrix)

# Add the outcomes and binary columns
scaled_data$Connectivity.density <- outputs$Connectivity.density
...
write.csv(scaled_data, "connectivity_scaled.csv", row.names = FALSE)
...
# Train the model for Connectivity Density
model <- glmnet(inputs_matrix, outputs_matrix, family = "gaussian", alpha = alpha_val)

# Save the model
saveRDS(model, "model_connectivity_density.rds")
cat("Model for Bone Porosity saved as model_connectivity_density.rds\n")
...
write.csv(data.frame(Model = "connectivity_density", LambdaMin = lambda.min_inputs), "density_lambda.csv", row.names = FALSE)

coef(model, s = cvmodel$lambda.min)

# Save the cross-validated model
saveRDS(cvmodel, "cvmodel_connectivity_density.rds")
cat("Cross-validated model saved as cvmodel_connectivity_density.rds\n")
...
miRData <- read.csv("connectivity_scaled.csv", header = TRUE)
...
write.csv(df, file = "connectivity_predictions.csv", row.names = FALSE)
...
for (feature in features) {
  ...
  cor_test_actual <- cor.test(inputs[[feature]], scaled_data$Connectivity.density, method = "spearman")
  ...
  actual_plot <- ggplot(data.frame(x = inputs[[feature]], y = scaled_data$Connectivity.density), aes(x = x, y = y)) +
