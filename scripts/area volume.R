...
setwd("C:\\Users\\konst\\insybio\\OsteoNet\\area volume")
...
# Load the datasets
inputs <- read.csv("area_inputs.csv", header = TRUE, row.names = 1)
outputs <- read.csv("area_outputs.csv", header = TRUE, row.names = 1)

# Remove unwanted columns
#inputs <- inputs %>% select(-c(Binary))
...
# Train the model for Area/Volume
model <- glmnet(inputs_matrix, outputs_matrix, family = "gaussian", alpha = alpha_val)

# Save the model
saveRDS(model, "model_area_volume.rds")
cat("Model for Area/Volume saved as model_area_volume.rds\n")
...
write.csv(data.frame(Model = "area_volume", LambdaMin = lambda.min_inputs), "area_lambda.csv", row.names = FALSE)

coef(model, s = cvmodel$lambda.min)

# Save the cross-validated model
saveRDS(cvmodel, "cvmodel_area_volume.rds")
cat("Cross-validated model saved as cvmodel_area_volume.rds\n")
...
miRData <- read.csv("area_scaled.csv", header = TRUE)
...
#data <- fold(newinputs, k = kfold, cat_col = 'Binary', id_col = 'ID', method = "n_dist") %>% arrange(.folds)
...
write.csv(df, file = "area_predictions.csv", row.names = FALSE)
...
pred_vs_actual_plot <- ggplot(data.frame(Out, PTOT), aes(x = Out, y = PTOT)) +
  geom_point() +
...
# Plot residuals
residuals_plot <- ggplot(data.frame(Out, residuals), aes(x = Out, y = residuals)) +
  geom_point() +
...
grid.arrange(pred_vs_actual_plot, residuals_plot, ncol = 2)
...
for (feature in features) {
  # Calculate Spearman correlation and p-value for actual outcomes
  cor_test_actual <- cor.test(inputs[[feature]], scaled_data$Area.volume, method = "spearman")
  spearman_corr_actual <- cor_test_actual$estimate
  pval_actual <- cor_test_actual$p.value
  
  # Calculate Spearman correlation and p-value for predicted outcomes
  cor_test_predicted <- cor.test(inputs[[feature]], combined_data$Predicted, method = "spearman")
  spearman_corr_predicted <- cor_test_predicted$estimate
  pval_predicted <- cor_test_predicted$p.value
  
  actual_plot <- ggplot(data.frame(x = inputs[[feature]], y = scaled_data$Area.volume), aes(x = x, y = y)) +
    geom_point(color = "blue") +
    geom_smooth(method = "lm", se = FALSE, color = "blue") +
    labs(title = paste("[ Actual Outcome vs", feature," ]", "\nSpearman correlation:", round(spearman_corr_actual, 3), 
                       "\np-value:", format(pval_actual, digits = 3)), 
  ...
  predicted_plot <- ggplot(data.frame(x = inputs[[feature]], y = combined_data$Predicted), aes(x = x, y = y)) +
    ...
    labs(title = paste("[ Predicted Outcome vs", feature," ]", "\nSpearman correlation:", round(spearman_corr_predicted, 3), 
                       "\np-value:", format(pval_predicted, digits = 3)), 
...
# Combine and print the actual and predicted outcome plots
for (feature in features) {
  grid.arrange(plot_list_actual[[feature]], plot_list_predicted[[feature]], ncol = 2)
}
...
spearman_test <- cor.test(scatter_data$Actual, scatter_data$Predicted, method = "spearman")
rho <- round(spearman_test$estimate, 3)
p_value <- signif(spearman_test$p.value, 3)

# Create scatterplot for all predictions vs actual values
ggplot(scatter_data, aes(x = Predicted, y = Actual)) +
  geom_point(color = "blue") +
  geom_smooth(method = "lm", se = FALSE, color = "red") +  # Add regression line
  labs(
    title = paste("Actual vs Predicted Scatterplot\nRho:", rho, " | p-value:", p_value),
    x = "Predicted Values",
    y = "Actual Values"
  ) +
  theme_minimal()
