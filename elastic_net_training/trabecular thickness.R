...
setwd("C:\\Users\\konst\\insybio\\OsteoNet\\trabecular thickness")
...
# Load the datasets
inputs <- read.csv("thickness_inputs.csv", header = TRUE, row.names = 1)
outputs <- read.csv("thickness_outputs.csv", header = TRUE, row.names = 1)
...
write.csv(scaling_params, "thickness_scaling_parameters.csv", row.names = FALSE)

scaled_data <- data.frame(ID = row.names(inputs), inputs_matrix)

# Add the outcomes and binary columns
scaled_data$Trabecular.thickness <- outputs$Trabecular.thickness
...
write.csv(scaled_data, "thickness_scaled.csv", row.names = FALSE)
...
# Train the model for Trabecular Thickness
model <- glmnet(inputs_matrix, outputs_matrix, family = "gaussian", alpha = alpha_val)

# Save the model
saveRDS(model, "model_trabecular_thickness.rds")
cat("Model for Trabecular Thickness saved as model_trabecular_thickness.rds\n")
...
write.csv(data.frame(Model = "trabecular_thickness", LambdaMin = lambda.min_inputs), "thickness_lambda.csv", row.names = FALSE)

coef(model, s = cvmodel$lambda.min)

# Save the cross-validated model
saveRDS(cvmodel, "cvmodel_trabecular_thickness.rds")
cat("Cross-validated model saved as cvmodel_trabecular_thickness.rds\n")
...
miRData <- read.csv("thickness_scaled.csv", header = TRUE)
...
write.csv(df, file = "thickness_predictions.csv", row.names = FALSE)
...
for (feature in features) {
  ...
  cor_test_actual <- cor.test(inputs[[feature]], scaled_data$Trabecular.thickness, method = "spearman")
  ...
  actual_plot <- ggplot(data.frame(x = inputs[[feature]], y = scaled_data$Trabecular.thickness), aes(x = x, y = y)) +
