...
cvmodel <- readRDS("cvmodel_trabecular_thickness.rds")
scaling_params <- read.csv("thickness_scaling_parameters.csv")
...
outputs_unseen <- read.csv("thickness_unseen_labels.csv", header = TRUE, row.names = 1)
...
actuals <- outputs_unseen$Trabecular.thickness
...
write.csv(bootstrap_summary, "bootstrap_thickness_metrics.csv", row.names = FALSE)
