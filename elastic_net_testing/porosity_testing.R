...
cvmodel <- readRDS("cvmodel_bone_porosity.rds")
scaling_params <- read.csv("porosity_scaling_parameters.csv")
...
outputs_unseen <- read.csv("porosity_unseen_labels.csv", header = TRUE, row.names = 1)
...
actuals <- outputs_unseen$Bone.porosity
...
write.csv(bootstrap_summary, "bootstrap_porosity_metrics.csv", row.names = FALSE)
