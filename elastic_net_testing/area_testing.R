...
cvmodel <- readRDS("cvmodel_area_volume.rds")
scaling_params <- read.csv("area_scaling_parameters.csv")
...
outputs_unseen <- read.csv("area_unseen_labels.csv", header = TRUE, row.names = 1)
...
actuals <- outputs_unseen$Area.volume
...
write.csv(bootstrap_summary, "bootstrap_area_metrics.csv", row.names = FALSE)
