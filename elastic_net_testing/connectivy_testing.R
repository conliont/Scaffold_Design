...
cvmodel <- readRDS("cvmodel_connectivity_density.rds")
scaling_params <- read.csv("density_scaling_parameters.csv")
...
outputs_unseen <- read.csv("connectivity_unseen_labels.csv", header = TRUE, row.names = 1)
...
actuals <- outputs_unseen$Connectivity.density
...
write.csv(bootstrap_summary, "bootstrap_connectivity_metrics.csv", row.names = FALSE)
